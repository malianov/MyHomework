...
public class BankomatAppExample {
    public static void main(String[] args) {
        ...
        note5000.takeMoney(new Money(15_901));
    }
}
...
class Money {
    ...
    public void setAmt(int amt) {
        if(amt > 0 && amt < 200_000 && amt % Note.R100 == 0) {
            ...
            throw new RuntimeException("The ammount of money have to be no more than 100 000 and cratna 100");
        }
    }
}
...
class NoteModule5000 extends NoteModule {
    void takeMoney(Money money) {
        ...
        if(countNote > 0) {
            System.out.println("Gave out " + countNote + " banknotes in " + Note.R5000);
        }
...
class NoteModule1000 extends NoteModule {
    void takeMoney(Money money) {
        ...
        if(countNote > 0) {
            System.out.println("Gave out " + countNote + " banknotes in " + Note.R1000);
        }
        if(remind > 0 && next != null) {
            next.takeMoney(new Money(remind));
        }
...
class NoteModule500 extends NoteModule {
    void takeMoney(Money money) {
        ...
        if(countNote > 0) {
            System.out.println("Gave out " + countNote + " banknotes in " + Note.R500);
        }
        if(remind > 0 && next != null) {
            next.takeMoney(new Money(remind));
        }
...
class NoteModule100 extends NoteModule {
    void takeMoney(Money money) {
        ...
        if(countNote > 0) {
            System.out.println("Gave out " + countNote + " banknotes in " + Note.R100);
        }
        if(remind > 0 && next != null) {
            next.takeMoney(new Money(remind));
        }
